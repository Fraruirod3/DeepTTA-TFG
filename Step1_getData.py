...
class GetData():
    def __init__(self):
        PATH = 'C:/Users/franc/Desktop/TFG/DeepTTC-main'

        rnafile = PATH + '/Cell_line_RMA_proc_basalExp.txt'
        smilefile = PATH + '/smile_inchi.csv'
        pairfile = PATH + '/GDSC_Data/GDSC2_fitted_dose_response_25Feb20.xlsx'
        drug_infofile = PATH + "/GDSC_Data/Drug_listTue_Aug10_2021.csv"
        drug_thred = PATH + '/GDSC_Data/IC50_thred.txt'
...
#--------------------------------------------
    def ByCancerEquilibrado(self, random_seed):
        # 1) Lectura y filtros iniciales
        drug_cell_df = pd.read_excel(self.pairfile)
        drug_cell_df = self._filter_pair(drug_cell_df)

        # 2) Determina min_count = tamaño de la clase menos representada
        counts = drug_cell_df['TCGA_DESC'].value_counts()
        min_count = counts.min()
        cancer_types = counts.index.tolist()

        # 3) Baraja cada grupo y arranca con min_count
        groups = {
            c: grp.sample(frac=1, random_state=random_seed).reset_index(drop=True)
            for c, grp in drug_cell_df.groupby('TCGA_DESC')
        }
        pointers = {c: min_count for c in cancer_types}

        # 4) Round-robin: toma min_count de cada uno
        sampled = []
        for c in cancer_types:
            sampled.extend(groups[c].iloc[:min_count].to_dict('records'))

        # 5) Sigue añadiendo uno a uno hasta 40000
        while len(sampled) < 40000:
            added = False
            for c in cancer_types:
                ptr = pointers[c]
                if ptr < len(groups[c]) and len(sampled) < 40000:
                    sampled.append(groups[c].iloc[ptr].to_dict())
                    pointers[c] += 1
                    added = True
            if not added:
                break  # todos los grupos exhaustos

        balanced_df = pd.DataFrame(sampled)

        # 6) Estadísticas tras el muestreo
        self._stat_drug(balanced_df)
        self._stat_cell(balanced_df)
        self._stat_cancer(balanced_df)

        print(balanced_df['TCGA_DESC'].value_counts())
...
#-----------------------------------------------------------------
    ...
    def ByDrug(self,random_seed):
        ...
        print(drug_cell_df['DRUG_ID'].value_counts())


        train_data,test_data = self._split(df=drug_cell_df,col='DRUG_ID',ratio=0.2,random_seed=random_seed)

        return train_data,test_data
    
    #---------------------------------------------
    def ByDrugEquilibrado(self, random_seed):
        # 1) Lectura y filtros iniciales
        drug_cell_df = pd.read_excel(self.pairfile)
        drug_cell_df = self._filter_pair(drug_cell_df)

        # 2) Determina min_count = tamaño de la clase menos representada
        counts = drug_cell_df['DRUG_ID'].value_counts()
        min_count = counts.min()
        drug_list = counts.index.tolist()

        # 3) Baraja cada grupo y arranca con min_count
        groups = {
        drug: grp.sample(frac=1, random_state=random_seed).reset_index(drop=True)
        for drug, grp in drug_cell_df.groupby('DRUG_ID')
        }
        pointers = {drug: min_count for drug in drug_list}

        # 4) Round-robin: toma min_count de cada uno
        sampled = []
        for drug in drug_list:
            sampled.extend(groups[drug].iloc[:min_count].to_dict('records'))
        ...
        while len(sampled) < 40000:
            ...
            for drug in drug_list:
                ptr = pointers[drug]
                if ptr < len(groups[drug]) and len(sampled) < 40000:
                    sampled.append(groups[drug].iloc[ptr].to_dict())
                    pointers[drug] += 1
                    added = True
            if not added:
                break  # todos los grupos exhaustos
        ...
        print(balanced_df['DRUG_ID'].value_counts())
